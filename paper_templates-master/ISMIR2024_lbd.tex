...
% ------
\title{Zero-shot DJ Tool discovery using Speech Activity and pretrained CLAP embeddings}
...
%
\begin{abstract}
The abstract should be placed at the top left column and should contain about 150-200 words. LBD submissions are submitted as extended abstracts and \textcolor{red}{should not be anonymized}. Please note that extended abstract is not a conference paper. Therefore, it will not be part of the official ISMIR \conferenceyear\ proceedings, and should not be cited as such. 
\end{abstract}
%
...
\footnote{\texttt{http://github.com:ruohoruotsi/acapella-instrumental-no-drums-classification}}

%
\section{Crate digging, the Amen Break \& a short history of DJ tool}
...
We adopt a ``(2+1)-page policy'' for Late-Breaking Demo (LBD) papers in ISMIR \conferenceyear. That is, LBD papers are extended abstracts that may have a maximum of 2 pages of technical content (including figures and tables) \textcolor{red}{with one optional page that contains only references and acknowledgments.} 
...
\textcolor{red}{Note that LBD screening process is single-blind, and the authors' names should not be anonymized.}
...
Line numbers should be included in your originally submitted manuscript, for reference during screening. \textcolor{red}{However, after your extended abstract is accepted, you must remove all line numbers from the final camera-ready version.} This can be done in \LaTeX by commenting out the command \verb|\linenumbers|. This can be done in Microsoft Word by selecting Layout > Line Numbers > None.
...
\begin{figure}
 \centerline{\framebox{
 \includegraphics[alt={ISMIR 2024 LBD template test image},width=0.9\columnwidth]{figure.png}}}
